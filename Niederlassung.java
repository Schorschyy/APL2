...
public class Niederlassung extends Firmenstandort {
	...
	public void anfordern() {
		String prod_anfordern;
		Scanner anf_scanner = new Scanner(System.in);
		do {
			System.out.println("Produktion anfordern?");
			prod_anfordern = anf_scanner.next();
			
			if( prod_anfordern.equals ("j")) {
				this.produktionAngefordert=true;
			}
			
			if( prod_anfordern.equals ("n")) {
				this.produktionAngefordert=false;
			}
			
		}while(!(prod_anfordern.equals("j")) && !(prod_anfordern.equals ("n")));
